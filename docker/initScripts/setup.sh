...
CREATE TABLE users (
    id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- Unikalny identyfikator użytkownika
    username VARCHAR2(50) NOT NULL UNIQUE,                  -- Nazwa użytkownika (musi być unikalna)
    password_hash VARCHAR2(255) NOT NULL,                   -- Hasło w formie zaszyfrowanej
    employee_id NUMBER NOT NULL,                            -- Identyfikator powiązany z pracownikiem
    role VARCHAR2(50) NOT NULL                              -- Rola użytkownika (np. 'USER', 'ADMIN')
);
